...
class MovieService {
  ...
  async searchMovies(query) {
    const response = await fetch(`${this.baseUrl}search/movie?api_key=${this.apiKey}&language=id-ID&query=${query}&page=1`);

    if (!response.ok) {
      throw new Error('Pencarian film tidak ditemukan');
    }
